...
# ──────────────────────────────────────────────
LOCAL_OPTIONS = [
    {"id": "1", "nome": "MINISTÉRIO PÃO DA VIDA — TÉRREO"},
]
...
# ──────────────────────────────────────────────
SCHEDULE_OPTIONS = {
    "1": {"dias_aula": "Segunda-Feira / Terça-Feira / Quarta-Feira", "horario": "9:00 às 16:30"},
}
...
# ──────────────────────────────────────────────
ADDRESS_OPTIONS = {
    "1": "📍Rua Pinto de Campos, 244 — Bairro Bento Ribeiro",
}
...
<head>
    ...
    <title>NAILS DESIGNER — MINISTÉRIO PÃO DA VIDA</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .wizard-page {
            ...
            margin: 0 auto; padding: 8px 0 18px; text-align: center;
        }

        /* ── Progress bar ── */
        .wizard-progress {
            margin: 18px auto 22px; padding: 18px;
        ...
        .wizard-label {
            padding: 12px 10px; border: 1px solid #c9daf8; border-radius: 18px;
            background: #fff; color: #2f5fb4; font-size: 0.92rem;
        ...
        .hero-subtitle, .panel-subtitle {
            margin: 0; color: #2f5fb4; font-size: 1.05rem; line-height: 1.55;
        }
        .hero-badge {
            display: inline-block; margin: 8px 0 14px;
            padding: 8px 20px; border-radius: 999px;
            background: #fff; border: 2px solid var(--cor-principal);
            color: var(--cor-principal); font-size: 1rem;
            font-weight: 800; letter-spacing: 0.04em;
        }
        .hero-highlights { display: grid; gap: 10px; margin-top: 16px; }
        .hero-highlight, .info-card, .review-box, .step-card {
            border-radius: 22px; border: 1px solid #d7e3fb;
            background: #fff; box-shadow: 0 8px 24px rgba(17,85,204,0.08);
        }
        .hero-highlight {
            padding: 12px 14px; color: #2f5fb4; font-size: 0.95rem; font-weight: 700;
        }
        .hero-highlight strong {
            display: block; color: var(--cor-principal); font-size: 1.15rem; margin-bottom: 4px;
        }
        ...
        .benefit-slide {
            ...
            font-size: 0.98rem; font-weight: 800; line-height: 1.45; text-align: center;
        ...
        .benefits-dot {
            ...
            background: #b7cbf4; transition: transform 0.2s ease, background 0.2s ease;
        }
        ...
        .readonly-field {
            background: #eef4ff !important; color: #2f5fb4 !important; font-weight: 700;
        }
        .input-with-action {
            display: grid; grid-template-columns: minmax(0,1fr);
        ...
        .review-item {
            ...
            align-items: center; column-gap: 8px; padding: 7px 9px;
            border-radius: 10px; background: var(--cor-clara); text-align: left;
        }
        ...
        .review-check {
            ...
            background: var(--cor-clara); color: #17386d; line-height: 1.45; text-align: left;
        }
        .review-check input {
...
<body data-start-step="{{ current_step }}">
        ...
        <div class="wizard-shell">
            <form id="wizard-form" method="POST"
                ...
                <section class="wizard-panel" data-step="index">
                    <div class="hero-grid">
                        <div class="hero-card">
                            <span class="hero-pill">MINISTÉRIO PÃO DA VIDA</span>
                            <h1 class="hero-title">NAILS DESIGNER</h1>
                            <div class="hero-badge">Transformando vidas através da Capacitação</div>
                            <p class="hero-subtitle">
                                Curso de qualificação profissional oferecido pela Prefeitura do Rio de Janeiro.
                            ...
                            <div class="hero-highlights">
                                ...
                                <div class="hero-highlight">
                                    ...
                                    <div class="benefits-slider" data-benefits-slider>
                                        <div class="benefits-viewport">
                                            ...
                                            <div class="benefit-slide">&#128218; Material Incluso</div>
                                            <div class="benefit-slide">&#127891; Certificado</div>
                                        </div>
                                ...
                                <!-- Sobre -->
                                <div class="hero-highlight">
                                    <strong>MINISTÉRIO PÃO DA VIDA</strong>
                ...
                <section class="wizard-panel" data-step="escolher">
                    <div class="step-card">
                        ...
                        <div class="step-grid step-grid--stacked">
                            ...
                            <!-- Local (único — pré-selecionado) -->
                            <div class="form-group">
                                <label for="local_id">Local</label>
                                <select id="local_id" name="local_id">
                                    {% for local_option in local_options %}
                                    <option value="{{ local_option.id }}"
                                        {% if form_data.get('local_id') == local_option.id %}selected{% endif %}>
                                        {{ local_option.nome }}
                                    </option>
                                    {% endfor %}
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select id="curso_id" name="curso_id">
                                    <option value="" {% if not form_data.get('curso_id') %}selected{% endif %}>
                                        Selecione um curso
                                    </option>
                                    {% for course in course_catalog %}
                                    <option value="{{ course.id }}"
                                        {% if form_data.get('curso_id') == course.id %}selected{% endif %}>
                                        {{ course.nome }}
                                    </option>
                                    {% endfor %}
                                </select>
                            ...
                            <div class="form-group full">
                                ...
                                <select id="opcao_id" name="opcao_id">
                                    <option value="" {% if not form_data.get('opcao_id') %}selected{% endif %}>
                                        Selecione uma turma
                                    </option>
                                    {% for option in course_options %}
                                    <option value="{{ option.id }}"
                                        {% if form_data.get('opcao_id') == option.id %}selected{% endif %}>
                                        {{ option.turma }}
                                    </option>
                                    {% endfor %}
                                </select>
                        ...
                        <div class="review-layout">
                            ...
                            <div class="review-box full">
                                <div class="review-info-text"
                                     style="margin-bottom:10px; color:#2f5fb4; font-size:0.98rem; text-align:left;">
                                    <strong>Elegibilidade:</strong>
                                    Este curso é destinado a pessoas com 16 anos ou mais
                                    interessadas em qualificação profissional.
                                </div>
                                ...
                                <label class="review-check" for="confirma_dados">
                                    ...
                                    <span>
                                        Confirmo que tenho 16 anos ou mais e interesse em participar
                                        do curso selecionado.<br>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            /* ── Lógica de seleção ── */
            function atualizarTurmas(cursoId, selectedOpcaoId){
                ...
                ph.selected = !selectedOpcaoId;
                optionSelect.appendChild(ph);
            ...
            function aplicarOpcao(opcaoId){
                ...
                if (!op){
                    localInput.value=''; cursoInput.value=''; turmaInput.value='';
            ...
            courseSelect.addEventListener('change', function(){
                setError('curso_id','');
                atualizarTurmas(courseSelect.value, '');
                syncReview();
            });
            ...
            function validarPassoEscolher(){
                if(!courseCatalogById[String(courseSelect.value)]){setError('curso_id','Selecione um curso válido.');courseSelect.focus();return false;}
                if(!courseOptionsById[String(optionSelect.value)]){setError('opcao_id','Selecione uma turma válida.');optionSelect.focus();return false;}
                return true;
            }
            ...
            ['nome','genero','whatsapp','cep','bairro','email','como_conheceu'].forEach(function(id){
            ...
            if(localSelect){localSelect.addEventListener('change',function(){syncReview();});}

            ['local_id','curso_id','opcao_id'].forEach(function(id){
                var f=document.getElementById(id);
                if(f){f.addEventListener('input',syncReview);f.addEventListener('change',syncReview);}
            });
            ...
            function initBenefitsSlider(slider){
                ...
                var timer;
                var dots=slides.map(function(_,i){var dot=document.createElement('button');dot.type='button';dot.className='benefits-dot';dot.setAttribute('aria-label','Benefício '+(i+1));dot.addEventListener('click',function(){show(i);restart();});dotsHost.appendChild(dot);return dot;});
            ...
            if(initCursoId){
                ...
                atualizarTurmas(initCursoId,initOpcaoId);
            }
...
<head>
    ...
    <style>
        ...
        .wizard-progress { margin:12px auto 16px; padding:14px; border-radius:28px; background:rgba(255,255,255,0.9); box-shadow:0 12px 30px rgba(17,85,204,0.12); }
...
def validate_form_data(form_data):
    ...
    if (not selected_option
            ...
            or selected_option["curso_id"] != form_data["curso_id"]):
...
def error_step(errors):
    ...
    if any(k in errors for k in ("local_id","curso_id","opcao_id")): return "escolher"
    return "dados"
